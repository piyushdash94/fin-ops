# finops — token accounting for Claude apps
...
```bash
python -m pytest tests/test_*.py   # 77 finops tests, no network required
```

Part of the repo's 172-test suite. It pins the pricing arithmetic against hand-computed values, so a
